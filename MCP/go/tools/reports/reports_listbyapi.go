...
func Reports_listbyapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/reports/byApi%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateReports_listbyapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_reports_byApi",
		mcp.WithDescription("Lists report records by API."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reports_listbyapiHandler(cfg),
	}
}
