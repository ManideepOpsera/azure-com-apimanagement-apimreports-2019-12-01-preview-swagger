...
func Reports_listbytimeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/reports/byTime%s", cfg.BaseURL, resourceGroupName, serviceName, subscriptionId, queryString)
...
func CreateReports_listbytimeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_reports_byTime",
		mcp.WithDescription("Lists report records by Time."),
		...
		mcp.WithString("interval", mcp.Required(), mcp.Description("By time interval. Interval must be multiple of 15 minutes and may not be zero. The value should be in ISO  8601 format (http://en.wikipedia.org/wiki/ISO_8601#Durations).This code can be used to convert TimeSpan to a valid interval string: XmlConvert.ToString(new TimeSpan(hours, minutes, seconds)).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reports_listbytimeHandler(cfg),
	}
}
